...
**モックライブラリについて調べるのは昔から好き**で、外部登壇や社内勉強会で[設計比較](https://speakerdeck.com/karamaru/motukuraiburariapurotibi-jiao)を行ったり、簡易的な[自作](https://github.com/karamaru-alpha/mocka)を行ったりしていました(ぜひ見てみてね！)。
...
go-dyno がどのようにメソッドを動的に生み出しているのかの調査がとても大変だったー 😭

詳しくは発表資料参照なのですが、go-dyno は「メソッドの振る舞いは動的に切り替えられない」 → 「関数の振る舞いは動的に切り替えられる」 → 「**メソッドから関数を呼び出して引数はメソッドのやつを覗き見しよう！** 」という作戦のライブラリです。
...
ということで、**[mockio/go-dyno 作者の ovechkin さん](https://github.com/ovechkin-dm)に直接連絡して質問**することにしました。GitHub に SNS リンクがなかったので見つけるのに少し骨が折れましたが、無事特定することに成功しました。オランダに住んでいる人のようです。
...
主に関数とメソッドで引数を共有するトリックについて、引数のレジスタ・スタック割り当てを利用していること、スタックが連続した時の offset が 16 バイトになることを丁寧に教えてくれました（16 バイトの部分は OS や Go バージョンにより異なりビルドタグで制御される）。

お話する過程で、スタックフレームの構造や、関数とメソッドの違いなどについての理解が大きく深まりました。
自分の物分かりが悪かったのだけれど、根気強く教えてくれて本当にありがとうございました！😭
...
発表資料を見て少しでもいいなと思ったら、ぜひ[スポンサー](https://github.com/sponsors/ovechkin-dmimage.png)してあげてください！自分もしました 👏
...
物量が多くて早口進行にはなりましたが、「このライブラリのここがかっこいいでしょ！」という熱量だったり、「Go の中身ってこうなってるんだね！」みたいな驚きを自分なりに共有できたかなと思います。
...
実務で使えるほどの信頼性があるのか？と問われると △ ですが、そんなことはかっこいいからどうでもいいのです！👏
...
特に補足はないのですが、やはり Tips パートが個人的にお気に入りの部分です ☺️
...
あと、go-dyno の図解は結構頑張りました！難しい題材だったから上手く伝えられたか不安だけど、なんかすごそう！という雰囲気だけ感じて貰えたらいいなぁ ☁️

特にクライマックスの、**レジスタとスタックの中身をハックして引数を覗き見するトリック**について、すごそう感が伝わると嬉しい 🔥
